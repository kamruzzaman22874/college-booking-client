...
const Login = () => {
    const { loginUser, googleLogin, githubLogin, resetPassword } = useContext(AuthContext)
    ...
    const handleGithub = () => {
        githubLogin()
            .then(result => {
                const githubSingnIn = result.user;
                console.log(githubSingnIn);
                navigate(from, { replace: true });
            })
            .catch(err => console.error(err));
    }
    ...
    return (
        <form onSubmit={handleLogin} className="login-form w-full mx-auto  mt-32">
            ...
            <div className="flex gap-8 py-5 justify-center">
                ...
                <FaGithub onClick={handleGithub} className="text-4xl cursor-pointer" />
            </div>
        </form>
    );
};
